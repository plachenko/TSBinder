...
<div class="flex bg-slate-600 h-full p-2">
    <form class="flex flex-1 flex-col gap-2" onsubmit={handleSubmit}>
        ...
        {#each tempObj.fields as field}
            ...
            {#if ["text", "color"].includes(field.type)}
                <input class={`${field.type == "color" ? "h-[50px]" : ""}`} type={field.type} id={field.name} name={field.name} bind:value={field.value} required>
            {:else if field.type === "emoji"}
            <button onclick={(e) => {e.preventDefault(); showEmojiMenu = !showEmojiMenu}}> &nbsp;</button>
            
            {#if showEmojiMenu}
            <!-- <EmojiMenu /> -->
            {/if}
            {:else if field.type === "textarea"}
            <textarea class="flex-1" id={field.name} name={field.name} bind:value={field.value}></textarea>
            {/if}
